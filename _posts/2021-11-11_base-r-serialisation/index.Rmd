...
### Logical, integer, and numeric vectors in RDS
...
What about the first line? Given everything we've seen previously it's pretty tempting to guess that it means something similar to the SEXPTYPE codes that we've seen earlier. Perhaps in the same way that numeric is SEXPTYPE 14 and logical is SEXPTYPE 10, maybe there's some sense in which a single string has a "SEXPTYPE" of 262153? Turns out that's not true, but the intuition isn't completely wrong. For now, let's pretend that 262153 encodes something "like" a SEXPTYPE, and come back to it later.
