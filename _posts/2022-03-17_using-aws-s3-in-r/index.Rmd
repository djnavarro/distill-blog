...
As you might hope, the README does in fact tell you something about how it's organised. In particular, it informs me that there's a single file storing all the metadata, encoded as a zipped csv file: 
...
This sounds like a good place to start. Let's see if I can download the metadata file `dwca-nsw_avh-v1.0.zip`:
...
Not surprisingly there's a predominance of NSW. Since I live in Newtown (a neighbourhood in Sydney) I'm going to find the subset of images in the National Herbarium data whose locality matches the string `"Newtown"`:
...
Okay now let's `map_chr()` over all the files listed in `filepaths`, saving the relevant herbarium image in each case:
...
Even the jpg files are fairly high resolution and I don't want this post to be too big, so I'll shrink them for this post. Here's my helper function:

```{r}
shrink_herbarium_image <- function(file) {
  on.exit(gc()) # prevents cache problem
...
and here's me using `walk()` to shrink every image and the smol version in the `tiny_images` folder:
...
The output here is a character vector containing names for the created files. Here's the contents of the `tiny_images` folder, but shown as the actual images rather than file names because that's prettier:^[I'm using the **bs4cards** package to display the images in this layout]
...
Visit https://aws.amazon.com/s3/ and click on the "Create an AWS Account" button:

```{r echo=FALSE}
knitr::include_graphics("screenshots/aws_s3_signup.png")
```

This will lead you through a pretty standard sign up process. You'll have to give an email address for the "root user" (that's you), specify a password, and so on. The free tier allows you 5GB of storage for 12 months, which is fairly convenient for "playing around" purposes! 

### Creating credentials

You can create access credentials for the root user, but that's not the wisest thing to do. Instead, what you want to do is create an "IAM user" (where "IAM" stands for "Identity and Access Management") and then supply credentials for that user. https://us-east-1.console.aws.amazon.com/iamv2/home#/users
...
The images I've been displaying throughout are all stored in the "tiny-herbs" bucket. I wrote a little helper function that 
...
The code I've been using to display the images throughout the post looks like this:
