...
shrink_herbarium_image <- function(file) {
  ...
  gc() # prevents cache resource expiry
}

images <- list.files("herbarium/images", full.names = TRUE)
walk(images, shrink_herbarium_image)
```

Here they are:

```{r, echo=FALSE}
tibble(images = list.files("herbarium/images", full.names = TRUE)) %>% 
bs4cards::cards(
  image = images, 
  width = 3,
  border_width = 0,
...
post_herbarium_image <- function(file) {
  cat("uploading", file, "...")
  out <- put_object(
    file = paste0("herbarium/images/", file),
    object = file, 
    bucket = "s3://tiny-herbs/",
    acl = "public-read"
  )
...
```{r, cache=TRUE}
images <- list.files("herbarium/images")
walk(images, post_herbarium_image)
```
