...
We can now take the pixels from the jasmines image and make them "flow" across the image. To do this, we'll need to reintroduce the colour information. We can do this using `full_join()` from the dplyr package. I've written a small convenience function `join_parts()` that performs the join only after removing the original `x` and `y` coordinates from the `jas` data. The reason for this is that the `pts` data now contains the positional information we need, so we want the `x` and `y` values from that data set. That's easy enough: we drop those coordinates with `select()` and then join the two tables using only the `id` column. See? I promised it would be useful!
