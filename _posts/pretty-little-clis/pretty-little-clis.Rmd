...
```{r cli-without-setup, eval=FALSE} 
cli_alert("I'm still here bitches, and I know everything. -A")
```

It throws an obscure error message that includes something like this^[If you're running a distill site with an RSS feed it may also write malformed characters into the index.xml file. Even after fixing the errors in my .Rmd file I had to delete the index.xml file and then rebuild.]
```
