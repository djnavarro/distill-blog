...
```{r display-message-structure-1, echo=FALSE}
insert_asciicast("message-structure-1")
```
...
```{r message-structure-2, eval=FALSE}
...
threatening_text()
```
```{r display-message-structure-2, echo=FALSE}
insert_asciicast("message-structure-2")
```
...
An important thing to remember about cli is that the text it produces is a *message*, so its visibility in R Markdown depends on the chunk option for messages. For this document I set my knitr defaults so that messages and warnings would be shown, but that isn't necessarily the case for every R Markdown template (e.g., distill articles). For this document I included this line in the very first chunk (which I hid from the output by setting `include=FALSE` for that chunk)

```{r knitr-defaults, eval=FALSE}
knitr::opts_chunk$set(message = TRUE, warning = TRUE)
```

Because I set my document up like that this works

```{r cli-without-suppression} 
cli_text("I'm still here bitches, and I know everything. -A")
```

but I could turn it off again simply by setting the `message=FALSE` chunk option or by using the `suppressMessages()` function. Based on this you might think that including cli output in an R Markdown document is pretty straightforward, but there are other issues. Suppose we wanted to use `cli_alert()` to send the same message. This fails inside an R Markdown document and produces an error^[If you're running a site with an RSS feed it may also write malformed characters into the index.xml file. Even after fixing the errors in my .Rmd file I had to delete the index.xml file and then rebuild.] that complains about `PCDATA invalid Char value`.

```{r setup-rmd-cli}
# enable crayon support for ANSI colours and formatting
options(crayon.enabled = TRUE)

# convert a string x with ANSI formatting to HTML and wrap 
# inside pre and code tags
ansi_to_html <- function(x, options) {
  paste0(
...
# tell knitr to apply the ansi_to_html() function to messages
knitr::knit_hooks$set(message = ansi_to_html)
```
