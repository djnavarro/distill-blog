...
*[Author note]* This post is the third in a series I have been writing on Apache Arrow and R, and probably works best if you've read the previous two. The first one is a [getting started post](https://blog.djnavarro.net/posts/2021-11-19_starting-apache-arrow-in-r/), and the second one talks about the [arrow dplyr backend](https://blog.djnavarro.net/posts/2022-01-18_binding-arrow-to-r/). I've tried to make this one self-contained, but it does assume you have a little bit of knowledge about what the **arrow** package does. If that's you, read on! If you're not at that point yet, hopefully the first two posts will get you there!
...
Then I decide to "copy the data into Arrow". I do that in a very predictable way using the `arrow_table()` function supplied by the **arrow** package: 
...
To understand why I'm so curious, consider the two objects I now have. The `magicians` data set is a "data frame" (a tibble, technically) stored in R. The `arrowmagicks` data set, however, is a pointer to a data structure stored in Arrow. That data structure is a "Table" object. Arrow Tables are roughly analogous to data frames -- both represent tabular data with columns that may be of different types -- but they are not the same. An act of *translation* has occurred, as is illustrated below: 

```{r}
#| echo = FALSE,
#| fig.align = "center",
#| fig.cap = "A miniature translation guide. On the left a data frame in R is shown: it is comprised of three columns (called variables), and we use the term 'element' to refer to a constituent part of the column. On the right is a Table in Arrow: it too is comprised of three columns (encoded as Chunked Arrays), and within each element of a chunked array there is a Scalar. The data structure that translates one into the other is called a Schema."

knitr::include_graphics("img/data-types-overview.png", dpi = 100)
```

Not only has the data frame itself been translated, the values stored in each of the columns have been translated. "Integer" columns in R are mapped to "int32" columns in Arrow, "Date" columns in R become "date32" columns in Arrow, and so on. There is a structure (labelled a "schema" in the image above) that instructs the **arrow** package how to control this translation, and the longer I think about it the more I realise there must be quite a lot going on here. Yet it all seems to work seamlessly, and it works both ways. I can pull the `arrowmagicks` data back into R and recover the original data:
...
That being said, it's also valuable to give the data analyst flexibility. And that means we're going to need to talk about schemas.
...
"translating between languages" is under-determined. Edge cases are left to the user because only the user knows the context.]
...
Organising the world into concepts (or data structures) is hard.^[Back my former life as an mathematical psychologist I studied this stuff for a living, and wrote an [absurd number of academic papers](https://djnavarro.net/papers/) on how people learn and represent categories. What I *wanted* to understand was the relationship between human and machine learning, between natural and artificial intelligence, and so on. What I *actually* learned is that humans and machines are both super weird.] We define structures that impose order on a chaotic world. The quirky little ontology at the start of this section is a small snippet taken from a very long monograph written by John Wilkins in 1668 in which he made a valiant (but doomed... oh so doomed) attempt to organise all the categories of thing and propose a mechanism by which we could describe them within a single universal language.

The idea of a universal language has a certain abstract appeal, but these things are constructed by humans, and humans have quite distinctive perspectives on the world. As a 17th century English gentleman, John Wilkins was no exception. As a 21st century Australian lady I'm noticing that there's no distinction between placental and marsupial mammals here, and... well, there's a marked lack of biological plausibility to any of this.
...
Universal languages are a myth. 

## Translating machine objects

Ontologies in natural languages are more complicated things than their equivalents in a programming language, but nevertheless the same principles hold. Every language comes with a collection of data structures, rules, and assumptions about how things "are supposed to work". Just like human languages, machine languages carve the world up into boxes, and those boxes don't always match up with one another. Some concepts just don't translate properly.^[To all the would be enthusiasts considering: I am begging you, please *do not* attempt to come at me with a "Turing-completeness" style gotcha, as if I were a first year undergraduate who just encountered algorithmic information theory for the first time. Do not. This post is not the place for a long discussion, so I will very quietly mention that the finite-length prefix required to specify the translation between languages is an $O(1)$ term (if I recall correctly), and when you leave the textbooks and encounter the world, $O(1)$ terms can be very, very big. Let it rest. Seriously. Writing a real world transpiler turns out to be actually quite tricky. Sheesh.]

So how do we construct data structures in Arrow that are appropriate translations of data structures in R, and vice versa? For the moment, let's keep things simple and assume that the mapping we're interested in has the same format as the one described at the start of the post: in R we want to encode the data as a data frame, and in Arrow we want to encode it as a Table. If so, the translation question is "simply"^[You know that something awful is coming right? I mean, the moment the author says it is "simply" a matter of doing blah blah blah in the first act, it always means there will be at least one open hellmouth spawning eldritch horrors left and right sometime around the second act.] a matter of how we encode the columns.

Well... sort of.

Not really.

Actually... we're not quite ready to start moving data around yet.

### Table, ChunkedArray, and Scalar

Before we get to "moving data around" part it's helpful to step back a little and recognise that R and Arrow are designed quite differently. For starters, the underlying library to which the **arrow** package provides bindings is written in C++, and C++ is itself a different kind of language than R. And in a sense, that's actually the natural place to start because it influences a lot of things in the design of **arrow**. Specifically, C++ and R take different approaches to object oriented programming (OOP).

The approach taken in C++ is [encapsulated OOP](https://adv-r.hadley.nz/oo.html) model that is common to many programming languages: methods belong to objects. In contrast, the approach taken in R is to have lots of different OOP systems that all work differently.^[This "let a thousand OOP systems bloom" philosophy has made me cry many times.] The most common one is S3, which is a [functional OOP](https://adv-r.hadley.nz/oo.html) model: methods belong to generic functions like `print()`. However there are also encapsulated OOP systems like [R6](https://r6.r-lib.org/) that work more like the OOP systems seen in other languages. 

The **arrow** package uses both S3 and R6, but it uses them for quite different things. Whenever **arrow** does something in an "R-native" way, S3 methods get used a lot. For example, in my earlier post on [dplyr bindings for Arrow](https://blog.djnavarro.net/posts/2022-01-18_binding-arrow-to-r/) I talked about how **arrow** supplies a **dplyr** engine: this works in part by supplying S3 methods for various **dplyr** functions that are called whenever a suitable Arrow object gets passed to **dplyr**. The interface between **arrow** and **dplyr** uses S3 because this context is "R-like". 

However, **arrow** has a second task, which is to interact with the Arrow C++ library, in which all the data structures are objects that use... encapsulated OOP. So... when **arrow** has to interact with Arrow, it's useful to be as "C++ like" as possible. That means using encapsulated OOP, and that in turn means that the interface between **arrow** and Arrow is accomplished using R6. 

What does that mean, in practice?

Well, among other things, it means that a lot of the "normal" R functions that **arrow** supplies are actually wrappers around an R6 method. To make it concrete, let's think about the `arrow_table()` function. At the start of the post I used this function to translate an R data frame into an Arrow Table, like this:

```{r, results='hide'}
arrow_table(magicians)
```

This is a natural way of thinking about things in R, but the `arrow_table()` function doesn't actually do the work. Instead, there is an R6 object called `Table`. I could have done the same thing like this:

```{r, results='hide'}
Table$create(magicians)
```

It's literally the same thing. And when I used the `schema()` function to create a schema? Well, there's an R6 object called `Schema`, and it too has a `create()` method. I could have accomplished the same thing by calling `Schema$create()`. There are R6 objects for `Dataset`, `RecordBatch`, `Array`, `ChunkedArray`, `Scalar`, and more. Each of these corresponds to a data structure in Arrow, and these R6 objects are the primary points of contact between R and Arrow. I will only talk about a few of these in this post because -- as you'll see -- the post gets very long even when I restrict myself to a few of them. 

Interacting with R6 objects is something that took me a while to get the hang of, and it's not something that feels "native" to most R programmers because we're used to functional OOP rather than encapsulated OOP. So to make your life easier, the **arrow** developers often provide handy wrapper functions that have names suspiciously similar to the R6 objects. You can use `chunked_array()` to call `ChunkedArray$create`, `schema()` to call `Schema$create()`, and `arrow_table()` to call `Table$create()`.^[In general R6 classes use [UpperCamelCase](https://en.wikipedia.org/wiki/Camel_case), and the [snake_case](https://en.wikipedia.org/wiki/Snake_case) functions with the same name provide an interface to the R6 class that feels more familiar to an R user. The `arrow_table()` function breaks the convention, but that's only because `table()` is already a base R function that does something very different.] However, there won't always be a wrapper function to solve your particular problem, so you'll often find yourself needing to use R6 methods. As the post goes on, you'll see me doing that from time to time. 
...
At long last we arrive at the point where I'm talking about the data values themselves, and the simplest kind of data to talk about are logical data. In R, logical data can take on three possible values: `TRUE` and `FALSE` are the two truth values, and `NA` is used to denote missing data.^[Should I take this opportunity to discuss the fact that this means that what R actually implements is a [three valued logic](https://en.wikipedia.org/wiki/Three-valued_logic)? No. No I should not. I am going to *think* it very loudly though.] In a moment I'll show you how to directly pass individual values from R to Arrow, but for the moment let's stick to what we know and pass an entire data frame across. Here's a tiny data frame, with one column of logical values:
...
Hm. Okay that's a little underwhelming as output goes? I'd like to see the actual values please. Happily the **arrow** package supplies a `$` operator for Arrow Table objects so we can extract an individual column from `tbl` the same way we can from the original R object `dat`. Let's try that:

```{r}
tbl$values
```

The output looks a little different to what we'd get when printing out a single column of a data frame, but it's pretty clear that we've extracted the right thing. A single column inside an Arrow table is stored as a chunked array, so this looks right. 

Yay us! 

At this point, it's handy to remember that the `arrow_table()` function that I used to move the data into Arrow is really just a wrapper that allows you to access some of the `Table` functionality without having to think about R6 objects. And I also mentioned there's a `ChunkedArray` object... and a `chunked_array()` wrapper function. In hindsight, I probably didn't need to bother creating the whole data frame. I could have created the chunked array directly:
...
That's a cleaner way of doing things. If you want a table, use `Table` and its wrappers. If you want a chunked array, use `ChunkedArray` and its wrappers. There's no need to over-complicate things. 

Speaking of which... later in the post, I'll often want to send single values to Arrow. In those cases I don't even want to create a chunked array, I just want to pass a scalar value across. And if what you want is a scalar, the right thing to do is use the `Scalar` object. There aren't any wrapper functions for `Scalar`, so I'll have to use `Scalar$create()` directly:
...
I would learn that it has been `r round(c(hedy_age))` days since [Hedy Lamarr](https://en.wikipedia.org/wiki/Hedy_Lamarr) was born.^[Yes, I looked up her [time and location of birth](https://www.astro.com/astro-databank/Lamarr,_Hedy) on an astrology website. I am, after all, a queer. I have been quite clear on this, and it is one of the traditions of our people.] More to the point, R has guessed that the length of time is sufficiently long that seconds aren't the appropriate encoding unit:
...
When I first started using **arrow**, the it was the magic I loved most. Everything just worked. I could move data between R and Arrow without having to think, I could manipulate enormous data sets using **dplyr** syntax that I'd never even be able to load into R, and I never had to look under the hood. Magic is always compelling. It is delightful. It makes the user feel joy, and it's the experience the developer wants to provide. 
...
I hope the post is helpful for anyone else following a similar path. 
