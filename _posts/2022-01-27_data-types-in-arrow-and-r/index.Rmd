...
There are no attributes attached to this object, it is simply a count of the number of seconds since that particular moment in time.^[This is a slight simplification: a POSIXct object is permitted to have a "tzone" attribute, a character string that specifies the timezone that is used when printing the object will be preserved when it is converted to a POSIXlt. However, it is optional and does not always get included.] The POSIXct object doesn't store the timezone information. The fact that it appears when I print out `now` occurs because the `print()` method for POSIXct objects prints the time with respect to a particular timezone. The default is to use the system timezone, which you can check by calling `Sys.timezone()`, but you can override this behaviour by specifying the timezone explicitly (for a list of timezone names, see `OlsonNames()`). So if I wanted to print the time in Berlin, I could do this:

```{r}
print(now, tz = "Europe/Berlin")
```

The important thing here is that the timezone is not inherently part of the POSIXct object: it belongs to the print method. Regardless of what you see when you print it out, the central idea in the POSIXct class is the same as in the Date class. These are numeric data structures that abstract away from *where* you are and the legal structures (timezones) that attache to your location. They are counts: Date objects count the number of days since the unix epoch, and POSIXct objects count the number of seconds. That's it.

### The POSIXlt class

What about POSIXlt? It turns out that this is a quite different kind of data structure, and it "thinks" about time in a very different waty. The "lt" in POSIXlt stands for "local time", and internally a POSIXlt object is a list that stores information about the time in a way that more closely mirrors how humans think about it. Here's what `now` looks like when I coerce it to a POSIXlt object:
...
The date64 type is similar to the date32 type, but instead of storing the number of days since 1970-01-01 as a 32-bit integer, it stores the number of milliseconds since 1970-01-01 00:00:00 UTC as a 64-bit integer. It's similar to the POSIXct class in R, except that:

- it uses milliseconds instead of seconds
- the internal storage is a 64-bit integer, not a 32-bit double
- it doesn't "hide" timezone information in metadata or print methods

Because it represents time at the millisecond level, the intended use of the date64 class is in situations where you want to keep track of units of time smaller than one day. I *can* create date64 objects from R Date objects if I want to...

```{r}
Scalar$create(today, date64())
```

...but this is quite wasteful. Why use a 64-bit representation that tracks time at the millisecond level when all I'm doing is storing the date? So let's create an Arrow analog of `now` as a date64 object:
...
The output is *printed* as a date, but again we can peek under the hood by explicitly casting this to a 64-bit integer:

```{r}
now_date64$cast(int64())
```

As a little reminder, this is what `now` looks like internally in R. It is essentially the same number, divided by 1000, because POSIXct stores the number of seconds whereas the Arrow date64 class stores the number of milliseconds.

```{r}
unclass(now)
```

This might strike you as very weird. On the face of it, what has happened is that I have taken `now` (which represents time at second-level precision), ported it over to Arrow, and created an object `now_date64` that apparently knows what millisecond it is???? How is that possible? Does Arrow have magic powers? 
...
Aha! Look at that decimal expansion. Turns out that even though `unclass(now)` quietly rounded the time to the nearest second, the original `now` variable was indeed a double, and it does store the time at subsecond accuracy. Ultimately, the accuracy of the time depends on the system clock itself, but the key thing to not here is that even though POSIXct times are almost always displayed to the nearest second, they do have the ability to represent more precise times. 
