...
```{r}
rmd_time_arrow <- Scalar$create(rmd_time)
rmd_time_arrow
rmd_time_arrow$type
```
...
```{r}
rmd_time_arrow <- Scalar$create(rmd_time, duration(unit = "us"))
rmd_time_arrow
rmd_time_arrow$type
```
...
```{r}
hms_time32_s <- Scalar$create(hms_time)
hms_time32_s
hms_time32_s$type
```
...
```{r}
hms_time32_ms <- Scalar$create(hms_time, time32(unit = "ms"))
hms_time32_ms
```
...
```{r}
hms_time64_us <- Scalar$create(hms_time, time64())
hms_time64_us
hms_time64_us$type
```
