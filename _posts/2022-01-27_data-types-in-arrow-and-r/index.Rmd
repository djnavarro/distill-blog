...
repack_double(unpack_double(12))
repack_double(unpack_double(1345234623462342))
repack_double(unpack_double(0.000000002345345234523))
```

We pretend that machines can do arithmetic. They can't, but it's a surprisingly powerful lie. 

### [Arrow] The float64 type

Okay. That was terribly long-winded, and I do apologise. Nevertheless, I promise there is a point to this story -- even the weird Baudrillard bit -- and it's time we switched back over to the Arrow side of things to think about what happens there.
...
- The Baudrillard quote emphasises that floating point numbers are a *simulation*. They are the mechanism by which we pretend that we have access to real numbers on computers. We don't, but it's a lie that almost works.
- The Quine quote emphasises that translation (and, I would argue, simulation also) is underdetermined. For any complicated thing there are many ways to simulate, or translate, or approximate it: all of them can be extremely good approximations, and they can *also* be inconsistent with one another. As it happens, this truism applies to floating point numbers too.

Remember when I was talking about floating point numbers and how they rely on a representation that looks like this:

$$
(\mbox{real number}) = (\mbox{sign}) \times (\mbox{significand}) \times 2 ^ {\mbox{(exponent)}}
$$

Why do we use 2 here? Programmers have a tendency to like using 2 as the base unit for everything because it lines up nicely with binary representations, and that's often a good instinct when dealing with machines. Unfortunately, it's an enormous problem for floating point arithmetic because *humans* write numbers in decimal notation (in the arabic number system anyway). Numbers that look simple in decimal notation often look complicated in binary notation and vice versa. As we saw earlier, a "simple" decimal number like 0.1 doesn't have a short binary expansion and so cannot be represented cleanly in a finite-precision binary floating point number system. Rounding errors are introduced every time a machine uses (base 2) floating point to encode data that were originally stored as a (base 10) number in human text.

The solution? Easy: Just use a floating point number system that uses 10 as the base:

$$
(\mbox{real number}) = (\mbox{sign}) \times (\mbox{significand}) \times 10 ^ {\mbox{(exponent)}}
$$
This is the core idea behind [decimal floating point](https://en.wikipedia.org/wiki/Decimal_floating_point) numbers. 

Arrow has two decimal types, a decimal128 type that (shockingly) uses 128 bits to store a floating point decimal number, and a decimal256 type that uses 256 bits. However, R doesn't have any analog of a decimal so I can't import an "R decimal object" to Arrow. However, what I can do is take the Arrow float64 objects I defined earlier and explicitly cast them to decimal format:

```{r}
decimal_01 <- float_01$cast(decimal128(5, 5))
decimal_01
decimal_01$type
```

When we use the decimal floating point type, we no longer encounter the bizarre problem of discovering that .1 + .2 is apparently not equal to .3. Everything works out nicely:

```{r}
decimal_02 <- float_02$cast(decimal128(5, 5))
decimal_03 <- float_03$cast(decimal128(5, 5))
decimal_01 + decimal_02 - decimal_03
```

The logical tests work out nicely too:

```{r}
decimal_01 + decimal_02 == decimal_03
```

What's important to keep in mind here, though, is that decimal types don't make floating point errors disappear. Decimal floating point numbers are still subject to all the weirdness of binary floating point numbers. However, what it does means is that the weirdness of the *machine-level* representation (decimal floating point) is neatly aligned with the weirdness of the *human-level* representation (decimal numbers). That comes in handy sometimes.
...
Okay, so as I mentioned, Arrow has two types.^[Once again, there's a little terminology to explain because there's some inconsistency in how the types are referred to in the Arrow C++ documentation and the **arrow** R package documentation. In the [list of data types](https://arrow.apache.org/docs/cpp/api/datatype.html) for the underlying C++ library you'll find references to "string" types and "large_string". However, in [list of data types](https://arrow.apache.org/docs/r/reference/data-type.html) documentation for the **arrow** R package you'll see the same data types referred to as "utf8" and "large_utf8".]
