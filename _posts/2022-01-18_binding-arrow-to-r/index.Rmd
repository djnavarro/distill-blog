...
```{r}
magicians <- read_csv("magicians.csv")
magicians
```
...
All of these computations take place within R. The `economics` data set is stored in R, and all the calculations are done using this data structure. What can we do when the data are stored in Arrow? Well, first I'll create an `arrowmagics` variable using the `arrow_read_csv()` function:
