...
## Prologue: My first contribution to Arrow
...
## Epilogue: What dplyr bindings are

The code in my pull request is basically this

```{r, eval=FALSE}
register_binding("leap_year", arrowish_leap_year)
```

You won't find `register_binding()` when looking through the **arrow** functions. It's only used internally. However, when you look at this simplified expression, it is hopefully not too difficult to work out what it does. The `register_binding()` function indicates that whenever the user calls `leap_year()` inside a **dplyr** pipeline, **arrow** will make sure that `arrowish_leap_year()` is the function that actually gets called. 

Want to know more about the *process* of getting this code incorporated into to the Arrow repository? Read the follow up to this post on ["My first contribution to Apache Arrow"](https://blog.djnavarro.net/contributing-to-arrow).
