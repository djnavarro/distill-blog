...
#| echo = FALSE,
#| fig.align = "center",
#| fig.cap = "Eliot. Image via [giphy](https://giphy.com/gifs/syfy-the-magicians-3o7ZePnuDJ1EiNsK3u), copyright [syfy](https://www.syfy.com)"
...
All of these computations take place within R. The `magicians` data set is stored in R, and all the calculations are done using this data structure. What can we do when the data are stored in Arrow? Well, first I'll create an `arrowmagics` variable using the `arrow_read_csv()` function:

```{r}
arrowmagics <- read_csv_arrow("magicians.csv", as_data_frame = FALSE)
arrowmagics
```

When I do this, two things happen. First, a data set is created outside of R (in memory allocated to Arrow) -- all of the computations will be done on that data set. Second, the `arrowmagics` variable is created inside R, which consists of a pointer to the actual data along with some handy metadata. The **arrow** package allows you to design **dplyr** pipelines using only this pointer object:
...
When the input is `arrowmagics`, the effect of the **dplyr** code is construct a query that can be passed to **libarrow** to be evaluated. This works because the **arrow** package supplies methods for `mutate()`, `group_by()`, and `summarise()` that are called whenever the input data is and Arrow Table. 
...
Let's imagine you're writing **dplyr** code to work with datetime data in a Table object like `arrowmagics`. If you were working with native R data like `magicians`, you can do something like this:
...
### Writing custom functions for Arrow pipelines {#arrowish-functions}
...
But that's not the only thing I've left unsaid. I haven't talked about unit tests, for example. I haven't talked about the social/technical process of getting code merged into the Arrow repository. If you've made it to the end of this post and are curious about joining the Arrow developer community, these are things you need to know about. I'll probably write something about those topics later on, but in the meantime here are two resources that might be handy:

- [Apache Arrow New Contributors Guide](https://arrow.apache.org/docs/dev/developers/guide/index.html)
- [Developers Guide to Writing Bindings](https://arrow.apache.org/docs/dev/r/articles/developers/bindings.html)

Please be aware that these are both under active development, so they may not be the best versions of themselves right now, and any feedback would certainly be appreciated.
