...
If I wanted to be clever I could modify the code in `head_expr` before allowing R to pull the trigger on evaluating it. I could combine a lot of expressions together, change parts of the code as needed, and evaluate them wherever I wanted. As you might imagine, this is super useful for creating domain specific languages within R, and **arrow** use it a lot. But this isn't a post about metaprogramming so let's evaluate it now:
...
arrowish_toupper <- function(x) {
  Expression$create("utf8_capitalize", x)
}
```
...
Before putting our functions into action, lets see what happens when we try to write a simply data analysis pipeline without them:
...
Yes, it returns the correct answer, but only because **arrow** detected a function it doesn't understand and "abandoned ship". It pulled the data into R and let **dplyr** do all the work. Now let's see what happens when we use our functions instead:
