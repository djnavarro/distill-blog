...
citation_url: https://blog.djnavarro.net/water-colours
...
<aside>
The data wrangling isn't shown, but it is in the [source code](https://github.com/djnavarro/distill-blog/tree/master/_posts/2021-09-07_generating-the-water-colours) 
</aside>
...
<aside>
For more information about the R environment see the [lockfile](https://github.com/djnavarro/distill-blog/tree/master/_posts/2021-09-07_generating-the-water-colours/renv.lock) for with this post. It's a story for another post, but for reproducibility purposes I have a separate [renv configuration](https://rstudio.github.io/renv/) for every post
</aside>
...
The next step in orientation is finding the post folder. Because this is a [distill](https://rstudio.github.io/distill/) blog, all my posts are stored in the `_posts` folder, and I've adopted a consistent [naming](https://slides.djnavarro.net/project-structure/) [convention](https://speakerdeck.com/jennybc/how-to-name-files) for my post folders. Every one begins with the post date in year-month-day format, followed by a human-readable "slug":
...
This allows me to construct the path to the image file and display it in the post:
...
<aside>
The `names_prefix` argument to `pivot_longer()` can do this without the extra step, but I prefer the verbose form. I think it's more readable and it handles `x` and `y` at the same code location
</aside>
...
<aside>
The [halftoner](https://github.com/cmarcum/halftoner) package will do this for you, incidentally
</aside>
...
The code above is inelegant. I've "iteratively" created a sequence of data frames by writing the same line of code several times. I could solve this problem by writing a loop, but I've become a fan of the functional programming tools in [purrr](https://purrr.tidyverse.org/) and I'll take advantage of that. In particular, the `purrr::accumulate()` function is an [underappreciated gem](https://adv-r.hadley.nz/functionals.html#accumulate). It does *precisely* the thing we want to do here. It takes one object (e.g., `points`) as input, together with a second quantity (e.g., an `amount`) and uses the user-supplied function (e.g., `shift()` to produce a *new* object that can again be passed to that function (i.e., new `points`). It continues with this process, taking the output of the last iteration of `shift()` and using it as input to the next iteration until it runs out of `amount` values. It is very similar to the better-known `purrr::reduce()` function, except that it doesn't throw away the intermediate values. The `reduce()` function is only interested in the destination; `accumulate()` is a whole *journey*. 
